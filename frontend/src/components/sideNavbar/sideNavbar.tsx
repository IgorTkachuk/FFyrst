...
const SideNavbar: React.FC<IProps> = ({ isCollapsed, setCollapse }) => {
  ...
  return (
    <>
      <ul
        className={`sidebar ${isCollapsed ? 'w-full' : ''} ${isCollapsed ? 'fixed left-0 top-0' : ''} ${isCollapsed ? '' : 'divide-y-2 divide-gray-300'} max-w-sidebar-content pt-20 px-1 lg:static bg-gray-100 content-area ring-2 ring-gray-300 `}>
